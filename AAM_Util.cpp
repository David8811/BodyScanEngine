...
void AAM_Pyramid::Draw(IplImage* image, const AAM_Shape& Shape, int type)
{
	...
	for(int j = 0; j < Shape.NPoints()-1; j++)	
		cvLine(image, cvPoint(Shape[j].x, Shape[j].y),cvPoint(Shape[j+1].x, Shape[j+1].y),CV_RGB(0, 255, 0),1);

		cvLine(image, cvPoint(Shape[Shape.NPoints()-1].x, Shape[Shape.NPoints()-1].y),cvPoint(Shape[0].x, Shape[0].y),CV_RGB(0, 255, 0),1);
